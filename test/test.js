...
describe('Test Issuance decoder', function () {
  ...
  it('should return Unlocked aggregatableasset ID from scriptSig.asm', function (done) {
    ...
    assert.equal(assetId1.startsWith('Ua'), true, 'Should be Unlocked and aggregatable')
    console.log(assetId(bitcoinTransaction))
    done()
  })

  it('should return Unlocked hybrid asset ID from scriptSig.asm', function (done) {
    bitcoinTransaction.cc_data[0].aggregationPolicy = 'hybrid'
    assetId1 = assetId(bitcoinTransaction)
    assert.equal(assetId1.startsWith('Uh'), true, 'Should be Unlocked and hybrid')
    console.log(assetId(bitcoinTransaction))
    done()
  })

  it('should return Unlocked dispersed asset ID from scriptSig.asm', function (done) {
    bitcoinTransaction.cc_data[0].aggregationPolicy = 'dispersed'
    assetId1 = assetId(bitcoinTransaction)
    assert.equal(assetId1.startsWith('Ud'), true, 'Should be Unlocked and dispersed')
    console.log(assetId(bitcoinTransaction))
    done()
  })
  ...
  it('should return Locked aggregatable asset ID', function (done) {
    ...
    bitcoinTransaction.cc_data[0].aggregationPolicy = 'aggregatable'
    assetId2 = assetId(bitcoinTransaction)
    assert.equal(assetId2.startsWith('La'), true, 'Should be Locked and aggregatable')
    console.log(assetId(bitcoinTransaction))
    done()
  })
  ...
  it('should return Locked hybrid asset ID', function (done) {
    bitcoinTransaction.cc_data[0].aggregationPolicy = 'hybrid'
    assetId2 = assetId(bitcoinTransaction)
    assert.equal(assetId2.startsWith('Lh'), true, 'Should be Locked and hybrid')
    console.log(assetId(bitcoinTransaction))
    done()
  })
  ...
  it('should return Locked dispersed asset ID', function (done) {
    bitcoinTransaction.cc_data[0].aggregationPolicy = 'dispersed'
    assetId2 = assetId(bitcoinTransaction)
    assert.equal(assetId2.startsWith('Ld'), true, 'Should be Locked and dispersed')
    console.log(assetId(bitcoinTransaction))
    done()
  })
  ...
  it('should return Locked asset ID', function (done) {
    ...
    assert.equal(assetId3.startsWith('La'), true, 'Should be Locked and aggregatable')
